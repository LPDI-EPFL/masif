...
with tf.python_io.TFRecordWriter(os.path.join(tfrecords_dir,'training_data_sequenceSplit_30.tfrecord')) as writer:
    for i, pdb in enumerate(train_pdbs):
        print('Working on', pdb)
        try:
            input_feat = np.load(os.path.join(precom_dir,pdb+'_','p1_input_feat.npy'))
            rho_wrt_center = np.load(os.path.join(precom_dir,pdb+'_','p1_rho_wrt_center.npy'))
            theta_wrt_center = np.load(os.path.join(precom_dir,pdb+'_','p1_theta_wrt_center.npy'))
            mask = np.load(os.path.join(precom_dir,pdb+'_','p1_mask.npy'))
            ...
            all_ligand_coords = np.load(os.path.join(ligand_coord_dir,'{}_ligand_coords.npy'.format(pdb.split('_')[0])))
            all_ligand_types = np.load(os.path.join(ligand_coord_dir,'{}_ligand_types.npy'.format(pdb.split('_')[0])))
        except:
            continue
        ...
        features_dict = {
            ...
            'theta_wrt_center_shape': tf.train.Feature(int64_list=theta_wrt_center_shape),
...
success = 0
with tf.python_io.TFRecordWriter(os.path.join(tfrecords_dir,'validation_data_sequenceSplit_30.tfrecord')) as writer:
...
success = 0
with tf.python_io.TFRecordWriter(os.path.join(tfrecords_dir,'testing_data_sequenceSplit_30.tfrecord')) as writer:
