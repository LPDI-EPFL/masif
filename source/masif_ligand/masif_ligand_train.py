...
with tf.Session() as sess:
    ...
    for num_epoch in range(num_epochs):
        ...
        for num_train_sample in range(int(num_training_samples/10)):
            ...
            training_loss, training_logits = learning_obj.session.run([learning_obj.data_loss, learning_obj.logits_softmax],feed_dict=feed_dict)
            training_losses.append(training_loss)
            training_ytrue.append(label)
            training_ypred.append(np.argmax(training_logits))

        print('Epoch {}, mean training loss {}, median training loss {}'.format(num_epoch,np.mean(training_losses),np.median(training_losses)))
        ...
        for num_val_sample in range(num_validation_samples):
            ...
            validation_loss, validation_logits = learning_obj.session.run([learning_obj.data_loss, learning_obj.logits_softmax],feed_dict=feed_dict)
            validation_losses.append(validation_loss)
            validation_ytrue.append(label)
            validation_ypred.append(np.argmax(validation_logits))

        print('Epoch {}, mean validation loss {}, median validation loss {}'.format(num_epoch,np.mean(validation_losses),np.median(validation_losses)))
        validation_conf_mat = confusion_matrix(validation_ytrue, validation_ypred)
        validation_accuracy = float(np.sum(np.diag(validation_conf_mat)))/np.sum(validation_conf_mat)
        ...
        for num_test_sample in range(num_testing_samples):
            ...
            testing_loss, testing_logits = learning_obj.session.run([learning_obj.data_loss, learning_obj.logits_softmax],feed_dict=feed_dict)
            testing_losses.append(testing_loss)
            testing_ytrue.append(label)
            testing_ypred.append(np.argmax(testing_logits))

        print('Epoch {}, mean testing loss {}, median testing loss {}'.format(num_epoch,np.mean(testing_losses),np.median(testing_losses)))
        testing_conf_mat = confusion_matrix(testing_ytrue, testing_ypred)
        testing_accuracy = float(np.sum(np.diag(testing_conf_mat)))/np.sum(testing_conf_mat)
        ...
        for num_sample in range(num_training_samples):
            ...
            if npoints<32:
                continue
            sample = np.random.choice(pocket_points,32,replace=False)
            feed_dict = {learning_obj.input_feat: data_element[0][sample,:,:],
                         learning_obj.rho_coords: np.expand_dims(data_element[1],-1)[sample,:,:],
                         learning_obj.theta_coords: np.expand_dims(data_element[2],-1)[sample,:,:],
                         learning_obj.mask: data_element[3][sample,:,:],
                         learning_obj.labels: pocket_labels,
                         learning_obj.keep_prob: 1.0
                        }

            _, training_loss, norm_grad, logits, logits_softmax, computed_loss = learning_obj.session.run([learning_obj.optimizer, 
                        learning_obj.data_loss, learning_obj.norm_grad, learning_obj.logits, learning_obj.logits_softmax, learning_obj.computed_loss],feed_dict=feed_dict)
            training_losses.append(training_loss)
            #training_ytrue.append(label)
            #training_ypred.append(np.argmax(logits_softmax))
            print('Num sample {}\tTraining loss {}\nLabels {}\tSoftmax logits {}\tComputed loss {}\n'.format(num_sample,training_loss,pocket_labels,logits_softmax,computed_loss))
            if num_sample%50==0:
                print('Mean training loss {}, median training loss {}'.format(np.mean(training_losses), np.median(training_losses)))
            total_iterations +=1
            if total_iterations == 40000:
                break
