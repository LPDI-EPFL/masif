...
def _parse_function(example_proto):
    keys_to_features = {'input_feat_shape': tf.FixedLenFeature([3], dtype=tf.int64),
    ...
    #'ligand' : tf.VarLenFeature(dtype=tf.string),
    ...
    input_feat = tf.reshape(input_feat,tf.cast(parsed_features['input_feat_shape'],tf.int32))
    rho_wrt_center = tf.sparse_tensor_to_dense(parsed_features['rho_wrt_center'])
    rho_wrt_center = tf.reshape(rho_wrt_center,tf.cast(parsed_features['rho_wrt_center_shape'],tf.int32))
    theta_wrt_center = tf.sparse_tensor_to_dense(parsed_features['theta_wrt_center'])
    theta_wrt_center = tf.reshape(theta_wrt_center,tf.cast(parsed_features['theta_wrt_center_shape'],tf.int32))
    ...
    labels = tf.reshape(labels,tf.cast(parsed_features['pocket_labels_shape'],tf.int32))
    return input_feat, rho_wrt_center, theta_wrt_center, mask, labels, parsed_features['pdb']
