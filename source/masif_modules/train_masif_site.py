...
def classify_iface_fp(params, learning_obj, rho_wrt_center, theta_wrt_center, input_feat, mask, indices, iface_labels):
    ...
    score = learning_obj.session.run([learning_obj.full_score],
                                          feed_dict=feed_dict) 
    return score 
...
def train_masif_site(learning_obj, params, 
            batch_size=100, num_iterations=100, num_iter_test=1000, batch_size_val_test=50):
    ...
    for num_iter in range(num_iterations):
                ...
                if ppi_pair_id in val_dirs:
                    ...
                    _, training_loss, norm_grad, score, eval_labels= \
                                        learning_obj.session.run([learning_obj.optimizer, \
                                                learning_obj.data_loss, learning_obj.norm_grad, learning_obj.eval_score, learning_obj.eval_labels\
                                                ],\
                                                      feed_dict=feed_dict) 
        ...
        outstr += 'Per protein AUC mean (training): {:.4f}; median: {:.4f} for iter {}\n'.format(np.mean(list_training_auc), np.median(list_training_auc), num_iter)
        outstr += 'Per protein AUC mean (validation): {:.4f}; median: {:.4f} for iter {}\n'.format(np.mean(list_val_auc), np.median(list_val_auc), num_iter)
        outstr += 'Per protein AUC mean (test): {:.4f}; median: {:.4f} for iter {}\n'.format(np.mean(list_test_auc), np.median(list_test_auc), num_iter)
        flat_all_test_labels= np.concatenate(all_test_labels, axis=0)
        flat_all_test_scores = np.concatenate(all_test_scores, axis=0)
        outstr += 'Testing auc (all points): {:.2f}'.format(metrics.roc_auc_score(flat_all_test_labels, flat_all_test_scores))
