...
def read_data_from_matfile(coord_file, shape_file, seed_pair, params, do_shape_comp_pairs, reshuffle=True):
    ...
    if True:
        ...
        for pair_ix in order[0:int(pairs_subset)]:
            ...
            s1, coord1 = extract_patch_and_coord(pix1, p_s['p1'], all_patch_coord['p1'], params['max_distance'], params['max_shape_size'])
            s2, coord2 = extract_patch_and_coord(pix2, p_s['p2'], all_patch_coord['p2'], params['max_distance'], params['max_shape_size'])
...
def read_data_from_matfile_full_protein(coord_file, shape_file, seed_pair, params, protein_id, label_iface=False, label_sc=False):
    ...
    all_patch_coord = {}
    print(coord_file)
    all_patch_coord[protein_id] = load_matlab_file(coord_file, ('all_patch_coord', protein_id))
...
#        sc_filter_val = np.asarray(shape_comp_25.todense())
#        sc_filter_val = np.percentile(sc_filter_val,50,axis=1)
    ...
    if label_iface:
        try:
            ...
            if protein_id == 'p1':
                ...
                plyfile = '{}_{}'.format(seed_pair.split('_')[0], seed_pair.split('_')[2])
    ...
    for pix in range(len(p_s[protein_id]['X'][0])):
        shape, coord, neigh_indices = extract_patch_and_coord(pix, p_s[protein_id], all_patch_coord[protein_id], params['max_distance'], params['max_shape_size'], patch_indices=True )
    ...
    if label_iface:
        return list_desc, list_coords, list_shape_idx, list_names, X, Y, Z, iface_labels, list_indices
    elif label_sc: 
        return list_desc, list_coords, list_shape_idx, list_names, X, Y, Z, sc_filter_val, list_indices
    else: 
...
def read_data_from_matfile_binding_pair(coord_file, shape_file, seed_pair, params, pid):
    ...
    all_patch_coord[other_pid] = load_matlab_file(coord_file, ('all_patch_coord', other_pid))
    ...
    v2 = np.vstack([p_s[other_pid]['X'][0], p_s[other_pid]['Y'][0],p_s[other_pid]['Z'][0]]).T
    ...
    list_vert_binder = []
    for vix in iface1:
        s, coord = extract_patch_and_coord(vix, p_s[pid], all_patch_coord[pid], params['max_distance'], params['max_shape_size'])
    ...
    list_vert_pos = []
    for vix in iface2:
        s, coord = extract_patch_and_coord(vix, p_s[other_pid], all_patch_coord[other_pid], params['max_distance'], params['max_shape_size'])
    ...
    return list_desc_binder, list_coord_binder, list_names_binder, list_vert_binder, labels1, list_desc_pos, list_coord_pos, list_names_pos, list_vert_pos, labels2

# Select points that are farther than X from the other protein. 
def read_data_from_matfile_negative_patches(coord_file, shape_file, ppi_pair_id, params, pid, other_pid):
    all_patch_coord = {}
    all_patch_coord[pid] = load_matlab_file(coord_file, ('all_patch_coord', pid))
    if all_patch_coord[pid] is None:
        return
    ...
    other_vert = np.vstack([p_s[other_pid]['X'][0], p_s[other_pid]['Y'][0],p_s[other_pid]['Z'][0]]).T
