function [Umin,Umax,Cmin,Cmax,Cmean,Cgauss,Normal] = compute_curvature(vertex,face, normalv, normalf,options)
...
% normals to faces
[tmp,normalf] = compute_normal(vertex,face,0);


% inner product of normals

dp = sum( normalf(:,E(:,1)) .* normalf(:,E(:,2)), 1 );
% angle un-signed
beta = acos(clamp(dp,-1,1));
% sign
cp = crossp( normalf(:,E(:,1))', normalf(:,E(:,2))' )';
...
[normalv,~] = compute_normal(vertex,face,0);
s = sign( sum(Normal.*normalv,1) ); 
