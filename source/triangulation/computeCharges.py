...
import numpy as np
import pyflann
from default_config.chemistry import polarHydrogens, radii, acceptorAngleAtom,\
                acceptorPlaneAtom, hbond_std_dev, donorAtom
            ...
            for atom2 in neigh_atoms:
                if atom2.get_id() == 'H':
                    ...
                    if res2.get_id() != res1.get_id():
                        ...
                        angle_N_H_O_dev = computeAngleDeviation(res2['N'].get_coord(),\
                                atom2.get_coord(), atom1.get_coord(), np.pi)
                        # Compute angle H:O=C, ideal value is ~160 +- 20 = 8*pi/9
                        angle_H_O_C_dev = computeAngleDeviation(atom2.get_coord(), \
                                atom1.get_coord(), res1['C'].get_coord(), \
                                8*np.pi/9)
