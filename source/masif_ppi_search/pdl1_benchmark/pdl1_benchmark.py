...
iface_dir = os.path.join(masif_root,'data/masif_site',masif_opts['site']['out_pred_dir'])
ply_iface_dir = os.path.join(masif_root,'data/masif_site',masif_opts['site']['out_surf_dir'])

desc_dir = os.path.join(top_dir,masif_opts['ppi_search']['desc_dir'])

pdb_dir = os.path.join(top_dir,masif_opts['pdb_chain_dir'])
precomp_dir = os.path.join(top_dir, masif_opts['ppi_search']['masif_precomputation_dir'])
...
target_patch, target_patch_descs = get_patch_geo(target_pcd,target_coord,target_vix,target_desc,flip=True, outward_shift=1.0)
...
def match_descriptors(in_desc_dir, in_iface_dir, pids, target_desc, desc_dist_cutoff=1.7, iface_cutoff=0.9 ):
...
def multidock(source_pcd,source_patch_coords,source_descs,cand_pts,target_pcd,target_descs, 
        ransac_radius=0.75, ransac_iter=4000):
    all_results = []
    all_source_patch = []
    all_source_scores = []
    patch_time = 0.0
...
parser = PDBParser()
target_struct = parser.get_structure('{}_{}'.format(target_pdb_id,target_chain), os.path.join(target_pdb_dir,'{}_{}.pdb'.format(target_pdb_id,target_chain)))
...
(matched_names, matched_vix, matched_desc_dist, count_proteins) = match_descriptors(desc_dir, iface_dir, ['p1', 'p2'], target_desc[target_vix])        
...
for name in matched_dict.keys():
    ...
    blockPrint()
    source_pcd = read_point_cloud(os.path.join(surf_dir,'{}.ply'.format(pdb+'_'+chain)))
    enablePrint()
#    print('Reading ply {}'.format(time.time()- tic))
    ...
    try:
        source_coords = subsample_patch_coords_fast(precomp_dir, ppi_pair_id, pid, cv=source_vix, frac=1.0)
    except:
...
#    print('Reading coords {}'.format(time.time()- tic))
    source_desc = np.load(os.path.join(desc_dir,ppi_pair_id,pid+'_desc_straight.npy'))
...
#    print('Multidock took {}'.format(time.time()- tic))
    ...
    if len(top_scorers) > 0:
        ...
        for j in top_scorers:
            ...
            source_struct = parser.get_structure('{}_{}'.format(pdb,chain), os.path.join(pdb_dir,'{}_{}.pdb'.format(pdb,chain)))
            res = all_results[j]
            ...
            align_and_save(out_fn, all_source_patch[j], res.transformation, source_struct, target_ca_pcd_tree,target_pcd_tree)                        
